  ...
  api: {
    ...
    waitinglist: {
      join(emailAddress) {
        ...
        return fetch(`${LANDING_PAGE_MODEL.api.baseUrl}/waitinglist/join?email=${email}&referredBy=${referredBy}`).then(function (res) {
...
function updateAppStateAndLocalStorage(info) {
  ...
  STATE.waitingList.info.referralCode = `https://proptee.io/?r=${info.refCode || '123'}`;
...
function createMessengerShareLink(referralCodeLink) {
  if (isMobile()) {
    ...
    const redirectUri = 'https://proptee.io/';
